...
uint32_t Chip_SPI_CalClkRateDivider(LPC_SPI_T *pSPI __attribute__ ((unused)), uint32_t bitRate)
{
	uint32_t SPIClk;
	uint32_t DivVal;
...
uint32_t Chip_SPI_RWFrames_Blocking(LPC_SPI_T *pSPI, SPI_DATA_SETUP_T *pXfSetup)
{
	...
	while ((pXfSetup->TxCnt < pXfSetup->Length) ||
		...
		_Status = Chip_SPI_GetStatus(pSPI);

		/* In case of TxReady */
		if ((_Status & SPI_STAT_TXRDY) && (pXfSetup->TxCnt < pXfSetup->Length)) {
			SPI_Send_Data(pSPI, pXfSetup);
		}

		/*In case of Rx ready */
		if ((_Status & SPI_STAT_RXRDY) && (pXfSetup->RxCnt < pXfSetup->Length)) {
...
Status Chip_SPI_Int_RWFrames(LPC_SPI_T *pSPI, SPI_DATA_SETUP_T *pXfSetup)
{
	...
	if (_Status & (SPI_STAT_RXOV | SPI_STAT_TXUR)) {
		/* Clear errors */
